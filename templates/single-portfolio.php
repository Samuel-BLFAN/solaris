...
if ( !function_exists( 'themerex_template_single_portfolio_output' ) ) {
	function themerex_template_single_portfolio_output($post_options, $post_data) {
		...
		if ($post_data['post_protected']) { 
			...
			if ( themerex_get_custom_option('show_post_tags') == 'yes' && (!empty($post_data['post_terms'][$post_data['post_taxonomy_tags']]->terms_links) || !empty($post_data['post_terms'][$post_data['post_taxonomy']]->terms_links))) {
				?>
				<div class="post_info post_info_bottom">
					<?php if(!empty($post_data['post_terms'][$post_data['post_taxonomy_tags']]->terms_links)) { ?>
						<span class="post_info_item post_info_tags"><?php _e('Tags: &nbsp &nbsp', 'themerex'); ?> <?php echo join('', $post_data['post_terms'][$post_data['post_taxonomy_tags']]->terms_links); ?></span>
					<?php }  ?>
					<?php if (!empty($post_data['post_terms'][$post_data['post_taxonomy']]->terms_links)) { ?>
						<span class="post_info_item post_info_category"><?php _e('Category: &nbsp &nbsp', 'themerex'); ?> <?php echo join('', $post_data['post_terms'][$post_data['post_taxonomy']]->terms_links); ?></span>
					<?php
					} ?>
