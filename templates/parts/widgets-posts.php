...
$output .= '<article class="post_item' . ($show_image == 0 ? ' no_thumb' : ($show_image == 2 ? ' with_icon' : ' with_thumb')) . ($post_number==1 ? ' first' : '') . '">';
...
else if ($show_image==2) {
	$output .= '<div class="post_icon"></div>';
}
...
if ($show_counters) {
	if (themerex_strpos($show_counters, 'views')!==false) {
		...
		$post_counters_icon = 'post_counters_likes icon-heart-2';
...
if ($show_date || $show_counters || $show_author) {
	...
	if ($show_author) {
		...
		$output .= '<span class="post_info_item post_info_posted_by">' . __('by', 'themerex') . ' ' . ($post_link ? '<a href="' . esc_url($post_author_url) . '" class="post_info_author">' : '') . ($post_author_name) . ($post_link ? '</a>' : '') . '</span>';
	}
