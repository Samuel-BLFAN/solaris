...
function themerex_sc_accordion($atts, $content=null){	
	...
	$THEMEREX_GLOBALS['sc_accordion_icon_closed'] = empty($icon_closed) || themerex_sc_param_is_inherit($icon_closed) ? "icon-plus-1" : $icon_closed;
	$THEMEREX_GLOBALS['sc_accordion_icon_opened'] = empty($icon_opened) || themerex_sc_param_is_inherit($icon_opened) ? "icon-minus-1" : $icon_opened;
...
function themerex_sc_accordion_item($atts, $content=null) {
	...
	if (empty($icon_closed) || themerex_sc_param_is_inherit($icon_closed)) $icon_closed = $THEMEREX_GLOBALS['sc_accordion_icon_closed'] ? $THEMEREX_GLOBALS['sc_accordion_icon_closed'] : "icon-plus-1";
	if (empty($icon_opened) || themerex_sc_param_is_inherit($icon_opened)) $icon_opened = $THEMEREX_GLOBALS['sc_accordion_icon_opened'] ? $THEMEREX_GLOBALS['sc_accordion_icon_opened'] : "icon-minus-1";
	$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
...
/*
[trx_audio url="http://solaris.themerex.dnw/wp-content/uploads/2014/12/Dream-Music-Relax.mp3" image="http://solaris.themerex.dnw/wp-content/uploads/2014/10/post_audio.jpg" title="Insert Audio Title Here" author="Lily Hunter" controls="show" autoplay="off"]
*/
...
function themerex_sc_button($atts, $content=null){	
	...
	$output = '<a href="' . (empty($link) ? '#' : $link) . '"'
		...
		. ' class="sc_button sc_button_' . esc_attr($type) 
				...
				. (!empty($class) ? ' '.esc_attr($class) : '')
				. ($icon!='' && $style != 'border' ? '  sc_button_iconed '. esc_attr($icon) : ($style == 'border' ? ' sc_button_iconed icon-right-open ' : ''))
...
function themerex_sc_contact_form($atts, $content = null) {
	...
	if($style == 'destination' && themerex_sc_param_is_off($custom)){
		$criterias_list = '';
		$criterias = themerex_get_custom_option('contacts_criterias_list');
		if (!empty($criterias)) {
			$criterias = explode(',', $criterias);
			$criterias_list .= '<select data-placeholder="'. __('تحب تروح فين', 'themerex') .'" class="destinations" name="criteria">';
			$criterias_list .= '<option value="" selected>'. __('تحب تروح فين', 'themerex') .'</option>';
			foreach($criterias as $val){
				$criterias_list .= '<option value="'.esc_html($val).'">'.esc_html($val).'</option>';
			}
			$criterias_list .= '</select>';
		}
		themerex_enqueue_formstyler();
		themerex_enqueue_script( 'jquery-picker', themerex_get_file_url('/js/picker/picker.js'), array('jquery'), null, true );
		themerex_enqueue_script( 'jquery-picker-date', themerex_get_file_url('/js/picker/picker.date.js'), array('jquery'), null, true );
		themerex_enqueue_script( 'jquery-picker-time', themerex_get_file_url('/js/picker/picker.time.js'), array('jquery'), null, true );
		themerex_enqueue_script( 'jquery-picker-start', themerex_get_file_url('/js/picker/picker.start.js'), array('jquery'), null, true );

		$output = '<div class="sc_contact_form sc_contact_form_order'
			...
			. (!themerex_sc_param_is_off($animation) ? ' data-animation="' . esc_attr(themerex_sc_get_animation_classes($animation)) . '"' : '')
			. '>'
			. '<form' . ($id ? ' id="' . esc_attr($id) . '"' : '') . ' data-formtype="order" method="post" action="' . esc_url($action ? $action : $THEMEREX_GLOBALS['ajax_url']) . '">'
			...
			. ($description ? '<p class="sc_contact_form_description">' . ($description) . '</p>' : '')
				. '<div class="label_over"><input id="sc_contact_form_email" type="text" name="email" placeholder="' . __('بريدك الإلكتروني', 'themerex') . '"></div>'
				. ($criterias_list ? $criterias_list : '')
				. '<div class="date_time">'
					. '<div class="form_date start">'
						. '<span class="icon"></span>'
						. '<input placeholder="' . __('من يوم ', 'themerex') . '" class="sc_contact_form_date js__datepicker" type="text" name="date_start">'
					. '</div>'
					. '<div class="form_date end">'
						. '<span class="icon"></span>'
						. '<input placeholder="' . __('ليوم', 'themerex') . '" class="sc_contact_form_date js__datepicker" type="text" name="date_end">'
					. '</div>'
				. '</div>'
				. '<select data-placeholder="'. __('بتسافر عشان ', 'themerex') .'" class="destinations" name="travelReason">>'
					  .'<option value="" selected>بتسافر عشان</option>'
					  .'<option value="تستجم">تستجم</option>'
					  .'<option value="تستكشف">تستكشف</option>'
					  .'<option value="تغامر">تغامر</option>'
					  .'<option value="تعيش مع مجتمع بعادات مختلفة">تعيش مع مجتمع بعادات مختلفة</option>'
					.'</select>'
				. '<div class="sc_contact_form_button"><button>' . __('ظبطلي برنامج', 'themerex') . '</button></div>'
		...
		$output = '<div ' . ($id ? ' id="' . esc_attr($id) . '"' : '')
			...
			. ($content != '' && themerex_sc_param_is_on($custom)
				...
				. '<div class="sc_contact_form_item sc_contact_form_button"><button>' . __('SEND MESSAGE', 'themerex') . '</button></div>'
			)
			. '<div class="result sc_infobox"></div>'
			. '</form>'
			. '</div>';
	...
	function themerex_sc_contact_form_send() {
		...
		if (!($contact_email = themerex_get_theme_option('contact_email')) && !($contact_email = themerex_get_theme_option('admin_email'))) 
			...
			if ($type_order=='order') {
				...
				$user_criteria	 = themerex_strshort($post_data['criteria'],	200);
				$user_date_start = themerex_strshort($post_data['date_start'],	100);
				$user_date_end 	 = themerex_strshort($post_data['date_end'],	100);

				$subj = sprintf(__('Site %s - Contact form message from %s', 'themerex'), get_bloginfo('site_name'), $user_email);
				$msg = "\n".__('E-mail:', 'themerex') .' '.esc_html($user_email)
					.  "\n".__('Destination:', 'themerex').' '.esc_html($user_criteria)
					.  "\n".__('Date start:', 'themerex').' '.esc_html($user_date_start)
					.  "\n".__('Date end:', 'themerex').' '.esc_html($user_date_end);
...
function themerex_sc_emailer($atts, $content = null) {
	...
	$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
			...
			. '<input type="text" class="sc_emailer_input" name="email" value="" placeholder="'.__('ENTER YOUR EMAIL ADDRESS', 'themerex').'">'
...
function themerex_sc_image($atts, $content=null){	
	...
	if (!empty($link)) {
		$output = empty($src) ? '' : (
			  '<div class="sc_image_wrap '. ($align && $align != 'none' ? ' align' . esc_attr($align) : ''). (!empty($class) ? ' ' . esc_attr($class) : '') .'"'
			  . (!themerex_sc_param_is_off($animation) ? ' data-animation="' . esc_attr(themerex_sc_get_animation_classes($animation)) . '"' : '')
			  . ($css1 != '' ? ' style="' . esc_attr($css1) . '"' : '')
			  .'>'
			. '<figure' . ($id ? ' id="' . esc_attr($id) . '"' : '')
			. ' class="sc_image ' . (!empty($shape) ? ' sc_image_shape_' . esc_attr($shape) : '') . '"'
			. ($css2 != '' ? ' style="' . esc_attr($css2) . '"' : '')
			. '>'
...
function themerex_sc_quote($atts, $content=null){	
	...
	$css .= (($bg_image !== '' && $style == '1') ? ' background-image:url(' . esc_url($bg_image) . ');' : '');
				...
				. ' title="' . __('Open/close search form', 'themerex') . '">
					...
					<div class="search_form_wrap">
						<form role="search" method="get" class="search_form" action="' . esc_url( home_url( '/' ) ) . '">
							<button type="submit" class="search_submit icon-zoom-1" title="' . __('Start search', 'themerex') . '"></button>
...
function themerex_sc_skills_item($atts, $content=null) {
	...
	if (empty($color)) $color = themerex_get_custom_option('accent_color');
	$color = apply_filters('themerex_filter_get_accent_color', $color);
	if (empty($bg_color)) $bg_color = $THEMEREX_GLOBALS['sc_skills_bg_color'];
	if (empty($bg_color)) $bg_color = '#63cac5';
	...
	if ($THEMEREX_GLOBALS['sc_skills_type'] == 'arc' || ($THEMEREX_GLOBALS['sc_skills_type'] == 'pie' && themerex_sc_param_is_on($THEMEREX_GLOBALS['sc_skills_pie_compact']))) {
		...
		$output .= 
				...
				. '</div>'
				. (($THEMEREX_GLOBALS['sc_skills_type']=='bar' || $THEMEREX_GLOBALS['sc_skills_type']=='bar2') && $THEMEREX_GLOBALS['sc_skills_dir']=='vertical' || $THEMEREX_GLOBALS['sc_skills_type'] == 'pie' ? $title_block : '')
...
function themerex_sc_tab($atts, $content = null) {
	...
	$output = '<div id="'.esc_attr($id).'"'
			...
			. (themerex_sc_param_is_on($THEMEREX_GLOBALS['sc_tab_scroll']) 
				? '<div id="'.esc_attr($id).'_scroll" class="sc_scroll sc_scroll_vertical" style="height:'.($THEMEREX_GLOBALS['sc_tab_height'] != '' ? $THEMEREX_GLOBALS['sc_tab_height'] : '160px').';"><div class="sc_scroll_wrapper swiper-wrapper"><div class="sc_scroll_slide swiper-slide">'
				: '')
			. do_shortcode($content) 
			. (themerex_sc_param_is_on($THEMEREX_GLOBALS['sc_tab_scroll']) 
	...
	if (themerex_sc_param_is_on($custom) && $content) {
		...
		while ( $query->have_posts() ) { 
			...
			if (empty($photo)) {
				...
				if($style == 1)
					$photo = themerex_get_resized_image_tag($photo, 350, 400);
				else if($style == 2)
					$photo = themerex_get_resized_image_tag($photo, 350, 350);
			}
			$socials = '';
			...
			if($style == 1) {
				...
				$output .= '<div class="column-1_' . esc_attr($columns) . '">'
					...
					. ($post_number == 1 ? ' first' : '')
					. '">'
					. ($link ? '<a class="sc_team_item_avatar" href="' . esc_url($link) . '">' :
					'<div class="sc_team_item_avatar">')
						. ($photo)
					. ($link ? '</a>' :
						'</div>')
...
function themerex_sc_testimonials($atts, $content=null){	
	...
	if (themerex_sc_param_is_on($custom) && $content) {
		...
		while ( $query->have_posts() ) { 
			...
			if (empty($photo)) {
				if (!empty($email))
					$photo = get_avatar($email, 200*min(2, max(1, themerex_get_theme_option("retina_ready"))));
			} else {
				$photo = themerex_get_resized_image_tag($photo, 200, 200);
			}
...
add_shortcode('trx_oblique', 'themerex_sc_oblique');

function themerex_sc_oblique($atts, $content=null){
	if (themerex_sc_in_shortcode_blogger()) return '';
	extract(shortcode_atts(array(
		"position" => "left",
		"bg_image" => "",
	...
	$css = "";
	$css .= themerex_get_css_position_from_values('!'.($top), '', '!'.($bottom));
	$css .= ($bg_image !== '' ? 'background-image: url('.themerex_get_file_url('/images/bg_oblique_'.esc_attr($position).'_'.esc_attr($size).'.png') . '), url(' . esc_url($bg_image) . ') ;' : '');

    $class .= ($position ? ' oblique_'.$position : '');
    $class .= ($size == 'big' ? ' bg_size_half' : '');

	$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
		. ' class="sc_section sc_oblique' . ($class ? ' '.esc_attr($class) : '') . '"'
		. (!themerex_sc_param_is_off($animation) ? ' data-animation="'.esc_attr(themerex_sc_get_animation_classes($animation)).'"' : '')
		. ($css!='' ? ' style="'.esc_attr($css).'"' : '').'>'
...
if($position == 'left'){
	...
	$output .= ($title_1 ? '<h4 class="sc_title">' .esc_attr($title_1) .'</h4>' : '');

	$output .= ($title_2 ? '<h3 class="sc_title">' .esc_attr($title_2) .'</h3>' : '');

	$output .= ($description ? '<div class="description">' .esc_attr($description) .'</div>' : '');

	$output .= ($info ? '<div class="info"><span class="info_time"></span>' .($info) .'</div>' : '');

	$output .= ($link ? '<a href="'.esc_url($link).'" class="sc_button sc_button_square sc_button_style_filled sc_button_bg_custom sc_button_size_large alignleft">' .esc_html($text_link) .'</a>' : '');
