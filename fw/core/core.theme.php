...
if ( !function_exists( 'themerex_get_logo_icon' ) ) {
	function themerex_get_logo_icon($slug) {
		...
		if (($logo_icon = themerex_get_custom_option($slug)) == '' && file_exists(themerex_get_file_dir('skins/' . ($skin) . '/images/' . ($slug) . '.png')))
...
if ( !function_exists( 'themerex_core_frontend_scripts' ) ) {
	function themerex_core_frontend_scripts() {
		...
		if (themerex_get_theme_option('packed_scripts')=='yes' && file_exists(themerex_get_file_dir('css/__packed.css'))) {
			// Load packed styles
		...
		if (themerex_get_theme_option('packed_scripts')=='yes' && file_exists(themerex_get_file_dir('js/__packed.js'))) {
...
if ( !function_exists( 'themerex_enqueue_slider' ) ) {
	function themerex_enqueue_slider($engine='all') {
		if ($engine=='all' || $engine=='swiper') {
			if (themerex_get_theme_option('packed_scripts')=='no' || !file_exists(themerex_get_file_dir('css/__packed.css'))) {
				themerex_enqueue_style( 'themerex-swiperslider-style', themerex_get_file_url('js/swiper/idangerous.swiper.css'), array(), null );
			}
			if (themerex_get_theme_option('packed_scripts')=='no' || !file_exists(themerex_get_file_dir('js/__packed.js'))) {
...
if ( !function_exists( 'themerex_enqueue_gallery' ) ) {
	function themerex_enqueue_gallery() {
		if (themerex_get_theme_option('packed_scripts')=='no' || !file_exists(themerex_get_file_dir('css/__packed.css'))) {
			themerex_enqueue_style( 'themerex-gallery-style',		themerex_get_file_url('js/gallery/component.css'), array(), null );
		}
		if (themerex_get_theme_option('packed_scripts')=='no' || !file_exists(themerex_get_file_dir('js/__packed.js'))) {
			themerex_enqueue_script( 'themerex-modernizr-script',	themerex_get_file_url('js/gallery/modernizr.min.js'),  array('jquery'), null, true );
			themerex_enqueue_script( 'themerex-classie-script',	themerex_get_file_url('js/gallery/classie.js'),  array('jquery'), null, true );
			themerex_enqueue_script( 'themerex-photostack-script',	themerex_get_file_url('js/gallery/photostack.js'),  array('jquery'), null, true );
		}
	}
}
...
//  Enqueue formstyler scripts
if ( !function_exists( 'themerex_enqueue_formstyler' ) ) {
	function themerex_enqueue_formstyler() {
		if (themerex_get_theme_option('packed_scripts')=='no' || !file_exists(themerex_get_file_dir('js/__packed.js'))) {
			themerex_enqueue_script( 'themerex-formstyler-script',	themerex_get_file_url('js/formstyler/jquery.formstyler.js'), array(), null, true  );
		}
	}
}
...
// Link must have attribute: data-rel="popup" or data-rel="popup[gallery]"
if ( !function_exists( 'themerex_enqueue_popup_login' ) ) {
	function themerex_enqueue_popup_login($engine='') {
		themerex_enqueue_style(  'themerex-magnific-style',	themerex_get_file_url('js/magnific/magnific-popup.css'), array(), null );
		themerex_enqueue_script( 'themerex-magnific-script',themerex_get_file_url('js/magnific/jquery.magnific-popup.min.js'), array('jquery'), '', true );
	}
}
...
if (!function_exists('themerex_core_add_scripts_inline')) {
	function themerex_core_add_scripts_inline() {
		...
		echo "<script type=\"text/javascript\">"
			...
			// Fix main menu
			. "THEMEREX_GLOBALS['menu_fixed']		= " . (themerex_get_custom_option('menu_position')=='fixed' ? 'true' : 'false') . ";"
