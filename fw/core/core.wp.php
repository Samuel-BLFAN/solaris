...
if (!function_exists('themerex_wp_theme_setup')) {
	...
	function themerex_wp_theme_setup() {
		...
		add_action('wp_ajax_clear_cache',					'themerex_callback_clear_cache');
		add_action('wp_ajax_nopriv_clear_cache', 			'themerex_callback_clear_cache');
	}
}
							...
							if ($level > $args['max_levels'])
								...
								$rez_all .= (!empty($rez_all) ? $args['delimiter'] : '') 
										. '<a class="breadcrumbs_item all" href="' . get_permalink($tpl_id) . '">' 
											. sprintf(__('All %s', 'themerex'), themerex_get_post_format_name($post_format, false)) 
										. '</a>';
						}
					}
...
if (!function_exists('themerex_show_pagination_single')) {
	function themerex_show_pagination_single( $args ) {
		...
		if ( is_single() ) {
			?>
			<h3 class="screen-reader-text"><?php _e( 'Post navigation', 'themerex' ); ?></h3>
			<?php
		...
		if ($page > 0 && is_array($args) && is_array($vars)) {
			...
			while ( have_posts() ) { the_post(); 
				...
				$post_args = array(
					...
					'terms_list' => !themerex_sc_param_is_off($vars['filters']) || themerex_get_template_property($vars['blog_style'], 'need_terms'),
					'strip_teaser' => false,
...
if (!function_exists('themerex_get_calendar')) {
	function themerex_get_calendar($onlyFirstLetter = false, $get_month = 0, $get_year = 0, $opt=array()) {
		...
		if ( $prev_year+$prev_month > 0 ) {
			$calendar_output .= '<a href="#" data-type="'.esc_attr(join(',', $posts_types)).'" data-year="' . esc_attr($prev_year) . '" data-month="' . esc_attr($prev_month) . '" title="' . esc_attr( sprintf(__('View posts for %1$s %2$s', 'themerex'), $wp_locale->get_month($prev_month), date('Y', mktime(0, 0, 0, $prev_month, 1, $prev_year)))) . '">'
		...
		if (count($posts) > (!empty($posts['done']) ? 1 : 0)) {
			...
			$link = '';
		$calendar_output .= '<th class="month_cur" colspan="5">' . ($link ? '<a href="' . esc_url($link) . '" title="' . esc_attr( sprintf(__('View posts for %1$s %2$s', 'themerex'), $wp_locale->get_month($thismonth), date('Y', mktime(0, 0, 0, $thismonth, 1, $thisyear)))) . '">' : '') . sprintf($calendar_caption, $wp_locale->get_month($thismonth), '<span>'.date('Y', $unixmonth).'</span>') . ($link ? '</a>' : '') . '</th>';
		...
		if ( $next_year+$next_month > 0 ) {
			$calendar_output .= '<a href="#" data-type="'.esc_attr(join(',', $posts_types)).'" data-year="' . esc_attr($next_year) . '" data-month="' . esc_attr($next_month) . '" title="' . esc_attr( sprintf(__('View posts for %1$s %2$s', 'themerex'), $wp_locale->get_month($next_month), date('Y', mktime(0, 0 , 0, $next_month, 1, $next_year))) ) . '">' 
		...
		for ( $day = 1; $day <= $daysinmonth; ++$day ) {
			...
			if ( !empty($posts[$day]) )
				$calendar_output .= '<a class="day_wrap" href="' . esc_url(!empty($posts[$day]['link']) ? $posts[$day]['link'] : get_day_link($thisyear, $thismonth, $day)) . '" title="' . esc_attr( is_int($posts[$day]['titles']) ? $posts[$day]['titles'].' '.__('items', 'themerex') : $posts[$day]['titles'] ) . "\">$day</a>";
			else
...
if (!function_exists('themerex_output_single_comment')) {
	function themerex_output_single_comment( $comment, $args, $depth ) {
		...
		switch ( $comment->comment_type ) {
			case 'pingback' :
				?>
				<li class="trackback"><?php _e( 'Trackback:', 'themerex' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( __( 'Edit', 'themerex' ), '<span class="edit-link">', '<span>' ); ?>
				<?php
				break;
			case 'trackback' :
				?>
				<li class="pingback"><?php _e( 'Pingback:', 'themerex' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( __( 'Edit', 'themerex' ), '<span class="edit-link">', '<span>' ); ?>
				<?php
				break;
			default :
				...
				<li id="comment-<?php comment_ID(); ?>" <?php comment_class('comment_item'); ?>>
					<div class="comment_author_avatar"><?php echo get_avatar( $comment, 140*min(2, max(1, themerex_get_theme_option("retina_ready")))); ?></div>
					<div class="comment_content">
						<div class="comment_info">
							<span class="comment_author"><?php echo ($author_id ? '<a href="'.esc_url($author_link).'">' : '') . comment_author() . ($author_id ? '</a>' : ''); ?></span>
							<span class="comment_date"><span class="comment_date_label"><?php _e('on', 'themerex'); ?></span> <span class="comment_date_value"><?php echo get_comment_date(get_option('date_format')); ?></span></span>
							<span class="comment_time"><?php echo get_comment_date(get_option('time_format')); ?></span>

							<?php if ($depth < $args['max_depth']) { ?>
								<span class="comment_reply"><?php comment_reply_link( array_merge( $args, array( 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) ); ?></span>
							<?php } ?>
...
if (!function_exists('themerex_detect_template_page_id')) {
	function themerex_detect_template_page_id() {
		static $template_id = '';
		if (!empty($template_id)) return $template_id;
		$key = themerex_detect_inheritance_key();
		if (!empty($key)) {
...
if ( !function_exists( 'themerex_show_taxonomies_filters' ) ) {
	function themerex_show_taxonomies_filters() {
		...
		if ($page == 'post')
			...
			return;
		echo trim(themerex_get_terms_filters($taxes));
	}
}
...
if (!function_exists('themerex_get_post_data_options')) {
	function themerex_get_post_data_options($opt) {
		$opt = array_merge(array(
			...
			'counters' => themerex_get_custom_option("blog_counters"),	// What counters use: views or comments
			...
			'sidebar' => !themerex_sc_param_is_off(themerex_get_custom_option('show_sidebar_main')),	// Sidebar is visible on current page
...
if (!function_exists('themerex_get_post_data')) {
	function themerex_get_post_data(&$opt, $post_obj=null) {
		...
		if ($opt['content']) {
			if ($opt['substitute_gallery'])	$post_content = themerex_substitute_gallery($post_content, $post_id, $thumb_sizes['w'], $thumb_sizes['h_crop'], 'none', true);
			$post_content = apply_filters('the_content', $post_content);
			if ($post_id != get_the_ID()) {		// Fix bug in the WPML
				$post = $cur_post;
				setup_postdata($post);
			}
			if ($opt['substitute_video']) 	$post_content = themerex_substitute_video($post_content, $thumb_sizes['w'], $thumb_sizes['h_crop']);
			if ($opt['substitute_audio'])	$post_content = themerex_substitute_audio($post_content);
		}
		...
		if ($opt['substitute_gallery']) $post_excerpt = themerex_substitute_gallery($post_excerpt, $post_id, $thumb_sizes['w'], $thumb_sizes['h_crop']);
		...
		if ($opt['substitute_video']) $post_excerpt = themerex_substitute_video($post_excerpt, $thumb_sizes['w'], $thumb_sizes['h_crop']);
		if ($opt['substitute_audio']) $post_excerpt = themerex_substitute_audio($post_excerpt);
		...
		$opt['dedicated'] = themerex_sc_get_dedicated_content();
		...
		if (!empty($opt['dedicated'])) {
			...
			if (!is_singular() || themerex_get_global('blog_streampage') || themerex_sc_in_shortcode_blogger(true) || (themerex_strpos($class, 'sc_align')!==false && themerex_strpos($class, 'columns')===false)) {
				$class = str_replace(array('sc_alignright', 'sc_alignleft', 'sc_aligncenter'), array('','',''), $class) . ' sc_align' . esc_attr($opt['location']);
				//if ($opt['location'] == 'center' && themerex_strpos($class, 'column-2_3')===false && $opt['sidebar'])
				//	$class = str_replace('columns', '_columns', $class) . ' column-2_3';
				//else if (($opt['location'] == 'left' || $opt['location'] == 'right') && themerex_strpos($class, 'column-1_2')===false)// && $opt['sidebar'])
		...
		if ($post_format == 'gallery') {
			...
			if ($post_video=='') {
				...
				if (!themerex_is_youtube_url($src) && !themerex_is_vimeo_url($src))
					$src = '';
			} else {
			...
			if ($post_video!='' && $opt['substitute_video']) {
				$post_video = themerex_substitute_video($post_video, $thumb_sizes['w'], round($thumb_sizes['w']/16*9), false);	//$thumb_sizes['h_crop']);
			}
...
if (!function_exists('themerex_get_post_views')) {
	function themerex_get_post_views($id=0){
		global $wp_query;
		if (!$id) $id = $wp_query->current_post>=0 ? get_the_ID() : $wp_query->post->ID;
...
if (!function_exists('themerex_set_post_views')) {
	function themerex_set_post_views($id=0, $counter=-1) {
		global $wp_query;
		if (!$id) $id = $wp_query->current_post>=0 ? get_the_ID() : $wp_query->post->ID;
		$count_key = 'post_views_count';
		$count = get_post_meta($id, $count_key, true);
...
if (!function_exists('themerex_get_post_likes')) {
	function themerex_get_post_likes($id=0){
		global $wp_query;
		if (!$id) $id = $wp_query->current_post>=0 ? get_the_ID() : $wp_query->post->ID;
...
if (!function_exists('themerex_set_post_likes')) {
	function themerex_set_post_likes($id=0, $count=0) {
		global $wp_query;
		if (!$id) $id = $wp_query->current_post>=0 ? get_the_ID() : $wp_query->post->ID;
		$count_key = 'post_likes_count';
		update_post_meta($id, $count_key, $count);
...
if ( !function_exists( 'themerex_callback_post_counter' ) ) {
	function themerex_callback_post_counter() {
		...
		if (isset($_REQUEST['likes'])) {
			...
			themerex_set_post_likes($id, $counter);
		} else if (isset($_REQUEST['views'])) {
			$counter = max(0, (int) $_REQUEST['views']);
			themerex_set_post_views($id, $counter);
		}
		echo json_encode($response);
		die();
	}
...
if ( !function_exists( 'themerex_callback_registration_user' ) ) {
	function themerex_callback_registration_user() {
		...
		if ( is_wp_error($id) ) {
		...
		} else if (($notify = themerex_get_theme_option('notify_about_new_registration'))!='no' && (($contact_email = themerex_get_theme_option('contact_email')) || ($contact_email = themerex_get_theme_option('admin_email')))) {
			$mail = themerex_get_theme_option('mail_function');
			if (in_array($notify, array('both', 'admin', 'yes'))) {
				$subj = sprintf(__('Site %s - New user registration: %s', 'themerex'), esc_html(get_bloginfo('site_name')), esc_html($user_name));
			...
			if (in_array($notify, array('both', 'user', 'yes'))) {
				$subj = sprintf(__('Welcome to "%s"', 'themerex'), get_bloginfo('site_name'));
				$msg = "\n".__('Your registration data:', 'themerex')
					."\n".__('Name:', 'themerex').' '.esc_html($user_name)
					."\n".__('E-mail:', 'themerex').' '.esc_html($user_email)
					."\n".__('Password:', 'themerex').' '.esc_html($user_pwd)
...
// AJAX: Login user
// add_action('wp_ajax_login_user',			'themerex_callback_login_user');
// add_action('wp_ajax_nopriv_login_user',	'themerex_callback_login_user');
if ( !function_exists( 'themerex_callback_login_user' ) ) {
	function themerex_callback_login_user() {
		global $_REQUEST;
		...
		$user_log = themerex_substr($_REQUEST['user_log'], 0, 60);
		$user_pwd = themerex_substr($_REQUEST['user_pwd'], 0, 20);
		$remember = themerex_substr($_REQUEST['remember'], 0, 7)=='forever';
...
if ( !function_exists( 'themerex_allow_email_login' ) ) {
