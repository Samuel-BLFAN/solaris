...
class WP_Import extends WP_Importer {
	...
	function import( $file ) {
		...
		@set_time_limit(max(30, min(1800, (int) themerex_get_theme_option('admin_dummy_timeout'))));
		...
		$result = $this->process_posts();
			...
			$error = true;
			echo '<br><br><b>'.sprintf(__('Error while import: %s', 'themerex'), $e->getMessage()).'</b><br>'; flush();
		}
...
if ( $this->debug ) { printf( __( 'Author "%s" imported.', 'themerex' ).'<br>', esc_html($this->authors[$old_login]['author_display_name']) ); flush(); }
	...
	function process_posts() {
		...
		foreach ( $this->posts as $post ) {
			...
			$admin_tm = max(30, min(1800, (int) themerex_get_theme_option('admin_dummy_timeout')));
			...
			themerex_fpc($this->import_log, trim($original_post_ID) . '|' . trim($result));
			printf( __('Post (ID=%s) imported. Current import progress: %s. Time limit: %s sec. Elapsed time: %s sec.', 'themerex'), $post['post_id'], $result.'%', $this->max_time, time() - $this->start_time);
			echo "<br /><br />";

			// Break import after timeout or if leave one post and execution time > half of max_time
			if (time() - $this->start_time >= $this->max_time || ($posts_counter + 1 >= $posts_all && time() - $this->start_time < min(60, $this->max_time/2))) break;
	...
	function process_menu_item( $item ) {
		...
		if ($_menu_item_type == 'custom' && strpos($_menu_item_url, $this->demo_url) !== false) {
			$_menu_item_url = str_replace($this->demo_url, get_site_url() . (themerex_substr($this->demo_url, -1)=='/' ? '/' : ''), $_menu_item_url);
		}
