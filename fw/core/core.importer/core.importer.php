...
class themerex_dummy_data_importer {
	...
	var $options = array(
		...
		'demo_url'				=> 'http://solaris.themerex.net/',	// URL of the demo site - need for change URLs in the custom menu items
	...
	function exporter() {
		...
		$this->export_options = base64_encode(str_replace('.dnw', '.net', serialize($options)));
		...
		$this->export_widgets = base64_encode(str_replace('.dnw', '.net', serialize($options)));
	...
	function importer() {
		...
		<p>&nbsp;</p>
		<div class="error">
			<h4><?php echo sprintf(__('Import progress: %s.', 'themerex'), $this->result.'%'); ?></h4>
		...
		if ($result>=100 && $this->options['import_to']) {
			themerex_options_reset();
			$this->import_theme_options();
		}

		// Import Widgets
		if ($result>=100 && $this->options['import_widgets'])	$this->import_widgets();

		// Import Sliders
		if ($result>=100 && $this->options['import_sliders'])	$this->import_sliders();

		// Setup Front page and Blog page
		if ($result>=100 && $this->options['import_posts']) {
			// Set reading options
...
if ($this->options['debug']) echo ' '.__('import error:', 'themerex').'<br>'.dumpVar($response);
						} else {
