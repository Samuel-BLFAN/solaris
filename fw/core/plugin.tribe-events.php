...
if (!function_exists('themerex_tribe_events_theme_setup')) {
	add_action( 'themerex_action_before_init_theme', 'themerex_tribe_events_theme_setup' );
	function themerex_tribe_events_theme_setup() {
		if (themerex_exists_tribe_events()) {
			...
			if (themerex_get_theme_option('show_overriden_posts')=='yes') {
				add_filter('manage_edit-'.TribeEvents::POSTTYPE.'_columns',			'themerex_post_add_options_column', 9);
				add_filter('manage_'.TribeEvents::POSTTYPE.'_posts_custom_column',	'themerex_post_fill_options_column', 9, 2);
			}
		}
	}
}
	...
	function themerex_tribe_events_get_blog_title($title, $page) {
		...
		if ( themerex_strpos($page, 'tribe')!==false ) {
			...
			if ( $page == 'tribe_category' ) {
				...
				$title = __( 'Tribe Events', 'themerex' );
			}
		}
		return $title;
	}
