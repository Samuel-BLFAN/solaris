...
class themerex_emailer {
	...
	function load_scripts() {
		if (isset($_REQUEST['page']) && $_REQUEST['page']=='trx_emailer') {
			themerex_enqueue_style('trx-emailer-style', themerex_get_file_url('tools/emailer/emailer.css'), array(), null);
		}
		if (isset($_REQUEST['page']) && $_REQUEST['page']=='trx_emailer') {
			...
			themerex_enqueue_script('trx-emailer-script', themerex_get_file_url('tools/emailer/emailer.js'), array('jquery'), null, true);
		}
	}
						...
						<div class="trx_emailer_fields">
							<div class="trx_emailer_field trx_emailer_group">
								...
								<select name="emailer_group" id="emailer_group">
									<option value="none"<?php echo $group=='none' ? ' selected="selected"' : ''; ?>><?php _e('- Select group -', 'themerex'); ?></option>
									<?php
								...
								<input type="checkbox" name="emailer_subscribers_update" id="emailer_subscribers_update" value="1"<?php echo $subscribers_update ? ' checked="checked"' : ''; ?> /><label for="emailer_subscribers_update" class="inline" title="<?php _e('Update the subscribers list for selected group', 'themerex'); ?>"><?php _e('Update', 'themerex'); ?></label>
								<input type="checkbox" name="emailer_subscribers_clear" id="emailer_subscribers_clear" value="1"<?php echo $subscribers_clear ? ' checked="checked"' : ''; ?> /><label for="emailer_subscribers_clear" class="inline" title="<?php _e('Clear this group from not confirmed emails after send', 'themerex'); ?>"><?php _e('Clear', 'themerex'); ?></label>
								<input type="checkbox" name="emailer_subscribers_delete" id="emailer_subscribers_delete" value="1"<?php echo $subscribers_delete ? ' checked="checked"' : ''; ?> /><label for="emailer_subscribers_delete" class="inline" title="<?php _e('Delete this group after send', 'themerex'); ?>"><?php _e('Delete', 'themerex'); ?></label>
							</div>
	...
	function themerex_callback_emailer_submit() {
		...
		if (preg_match('/[\.\-_A-Za-z0-9]+?@[\.\-A-Za-z0-9]+?[\ .A-Za-z0-9]{2,}/', $email)) {
			...
			if (isset($subscribers[$group][$email]))
				$response['error'] = __('E-mail address already in the subscribers list!', 'axiomthemes');
			else {
				$subscribers[$group][$email] = md5(mt_rand());
				update_option('themerex_emailer_subscribers', $subscribers);
				$subj = sprintf(__('Site %s - Subscribe confirmation', 'axiomthemes'), get_bloginfo('site_name'));
				...
				if (!@$mail($email, $subj, nl2br($msg), $headers)) {
					$response['error'] = __('Error send message!', 'axiomthemes');
				}
				remove_filter( 'wp_mail_content_type', 'themerex_set_html_content_type' );
			}
		} else
			$response['error'] = __('E-mail address is not valid!', 'axiomthemes');
		...
		if (preg_match('/[\.\-_A-Za-z0-9]+?@[\.\-A-Za-z0-9]+?[\ .A-Za-z0-9]{2,}/', $email)) {
			...
			if (isset($subscribers[$group][$email])) {
				if ($subscribers[$group][$email] == $nonce) {
					...
					update_option('themerex_emailer_subscribers', $subscribers);
					themerex_set_system_message(__('Confirmation complete! E-mail address succefully added in the subscribers list!', 'axiomthemes'), 'success');
					//header('Location: '.home_url());
					wp_safe_redirect( home_url() );
				} else if ($subscribers[$group][$email] != '') {
					themerex_set_system_message(__('Bad confirmation code!', 'axiomthemes'), 'error');
					//header('Location: '.home_url());
					wp_safe_redirect( home_url() );
				} else {
					themerex_set_system_message(__('E-mail address already exists in the subscribers list!', 'axiomthemes'), 'error');
					//header('Location: '.home_url());
					wp_safe_redirect( home_url() );
				}
			}
