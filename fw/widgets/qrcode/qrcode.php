...
class themerex_widget_qrcode extends WP_Widget {
	...
	function __construct() {
		...
		/* Create the widget. */
		parent::__construct( 'themerex_widget_qrcode', __('ThemeREX - QRCode generator', 'themerex'), $widget_ops, $control_ops );
			...
			$output .= '
					...
					' . ($show_what==1 
						? '<p class="user_name odd first"><span class="theme_text">' . __('Name:', 'themerex') . '</span> <span class="theme_info">' . ($ufname) . ' ' . ($ulname) . '<span></p>'
							...
							. ($uemail ? '<p class="user_email even"><span class="theme_text">' . __('E-mail:', 'themerex') . '</span> <a href="' . esc_url('mailto:'.($uemail)) . '">' . ($uemail) . '</a></p>' : '')
							. ($usite ? '<p class="user_site odd"><span class="theme_text">' . __('Site:', 'themerex') . '</span> <a href="' . esc_url($usite) . '" target="_blank">' . ($usite) . '</a></p>' : '')
						: $text)
						. '
					</div>
					';
	...
	function form($instance) {
		...
		themerex_enqueue_style( 'widget-qrcode-style',  themerex_get_file_url('widgets/qrcode/qrcode-admin.css'), array(), null );
		themerex_enqueue_script('jquery-ui-tabs', false, array('jquery','jquery-ui-core'), null, true);
		themerex_enqueue_script('widget-qrcode-script', themerex_get_file_url('widgets/qrcode/jquery.qrcode-0.6.0.min.js'), array(), null, false);
		...
		<div class="widget_qrcode">
        	<div class="qrcode_tabs">
                <ul class="tabs">
                    ...
                    <li><a href="#tab_fields" onmousedown="themerex_qrcode_init()"><?php _e('Personal Data', 'themerex'); ?></a></li>
                    <li><a href="#tab_text" onmousedown="themerex_qrcode_init()"><?php _e('Any Text', 'themerex'); ?></a></li>
                </ul>
				...
				<?php if ($instance['show_what']==1) { ?>
                    ...
                    <?php 
						echo  ($instance['ucompany'] ? '<li class="user_company even">' . __('Company:', 'themerex') . ' ' . ($instance['ucompany']) . '</li>' : '')
							...
							. ($instance['uemail'] ? '<li class="user_email even">' . __('E-mail:', 'themerex') . ' ' . '<a href="' . esc_url('mailto:'.($instance['uemail'])) . '">' . ($instance['uemail']) . '</a></li>' : '')
							. ($instance['usite'] ? '<li class="user_site odd">' . __('Site:', 'themerex') . ' ' . '<a href="' . esc_url($instance['usite']) . '" target="_blank">' . ($instance['usite']) . '</a></li>' : '');
					?>
				<?php } else { ?>
...
<?php } ?>
	...
	function load_scripts() {
		...
		themerex_enqueue_script('widget-qrcode-script', themerex_get_file_url('widgets/qrcode/jquery.qrcode-0.6.0.min.js'), array('jquery'), null, true);
	}
}
?>