...
class themerex_widget_top10 extends WP_Widget {
	...
	function __construct() {
		...
		/* Create the widget. */
		parent::__construct( 'themerex_widget_top10', __('ThemeREX - Top 10 Posts', 'themerex'), $widget_ops, $control_ops );

		// Add thumb sizes into list
		themerex_add_thumb_sizes( array( 'layout' => 'widgets', 'w' => 75, 'h' => 75, 'h_crop' => 75, 'title'=>__('Widgets', 'themerex') ) );
	}
	...
	function widget($args, $instance) {
		...
		if (count($tabs) > 0) {
			...
			if (count($tabs) > 1) themerex_enqueue_script('jquery-ui-tabs', false, array('jquery','jquery-ui-core'), null, true);
	...
	function form($instance) {
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('post_type')); ?>"><?php _e('Post type:', 'themerex'); ?></label>
		...
		<p>
			...
			<input type="radio" id="<?php echo esc_attr($this->get_field_id('show_image')); ?>_2" name="<?php echo esc_attr($this->get_field_name('show_image')); ?>" value="2" <?php echo ($show_image==2 ? ' checked="checked"' : ''); ?> />
			<label for="<?php echo esc_attr($this->get_field_id('show_image')); ?>_2"><?php _e('Show as icon', 'themerex'); ?></label>
		...
		<p>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_counters')); ?>_2"><?php _e('As heart', 'themerex'); ?></label>
