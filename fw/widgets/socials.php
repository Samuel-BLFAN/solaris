...
class themerex_widget_socials extends WP_Widget {
	...
	function __construct() {
		...
		/* Create the widget. */
		parent::__construct( 'themerex_widget_socials', __('ThemeREX - Show logo and social links', 'themerex'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		<div class="widget_inner">
            <?php
				if ($show_logo) {
					if ($logo_image=='')	$logo_image = themerex_get_custom_option('logo_image');
					if ($logo_text=='')		$logo_text  = themerex_get_custom_option('logo_text');
	...
	function form( $instance ) {
		...
		<p>
            ...
            <?php
			echo trim(themerex_show_custom_field($this->get_field_id( 'logo_media' ), array('type'=>'mediamanager', 'media_field_id'=>$this->get_field_id( 'logo_image' )), null));
			if ($logo_image) {
			?>
