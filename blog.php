...
$blog_style 	= themerex_get_custom_option('blog_style');
...
if (themerex_get_template_property($blog_style, 'need_isotope')) {
	if (!themerex_sc_param_is_off($show_filters)) {
		?>
		<div class="isotope_filters"></div>
		<?php
	}
	?>
	<div class="isotope_wrap <?php echo esc_attr(themerex_get_template_name($blog_style)); ?> <?php echo esc_attr(themerex_get_template_property($blog_style, 'container_classes')); ?>" data-columns="<?php echo esc_attr($blog_columns); ?>">
	<?php
	}
...
while ( have_posts() ) { the_post();
	...
	$post_args = array(
		...
		'terms_list' => !themerex_sc_param_is_off($show_filters) || themerex_get_template_property($blog_style, 'need_terms'),
		'parent_tax_id' => $parent_tax_id,
		'descr' => themerex_get_custom_option('post_excerpt_maxlength'.($blog_columns > 1 ? '_masonry' : '')),
		'sidebar' => !themerex_sc_param_is_off($show_sidebar),
...
if (!$post_number) {
	...
	if ($show_filters == 'categories') {			// Use categories as filter items
		...
		$portfolio_parent = $cur_term ? max(0, themerex_get_parent_taxonomy_by_property($cur_term->term_id, 'show_filters', 'yes', true, $taxonomy)) : 0;
		$args2 = array(
		...
		if (count($portfolio_list) > 0) {
			$filters .= '<a href="#" data-filter="*" class="'.esc_attr($filter_button_classes . ($portfolio_parent==$cur_term->term_id ? ' active' : '')) . '">' . __('All', 'themerex').'</a>';
	...
	if (in_array($pagination, array('viewmore', 'infinite'))) {
		if ($page_number < $wp_query->max_num_pages) {
			...
			<div id="viewmore" class="pagination_wrap pagination_<?php echo esc_attr($pagination); ?>">
				<a href="#" id="viewmore_link" class="sc_button sc_button_style_filled sc_button_bg_custom sc_button_size_large theme_button viewmore_button"><span class="icon-spin3 animate-spin viewmore_loading"></span><span class="viewmore_text_1"><?php _e('LOAD MORE', 'themerex'); ?></span><span class="viewmore_text_2"><?php _e('Loading ...', 'themerex'); ?></span></a>
				<span class="viewmore_loader"></span>
				<script type="text/javascript">
					jQuery(document).ready(function () {
						THEMEREX_GLOBALS['viewmore_page'] = <?php echo intval($page_number); ?>;
						THEMEREX_GLOBALS['viewmore_data'] = '<?php echo serialize($args); ?>';
						THEMEREX_GLOBALS['viewmore_vars'] = '<?php echo serialize(array(
